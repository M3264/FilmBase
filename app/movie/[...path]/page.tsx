...
export async function generateMetadata({ params }: { params: Promise<{ path: string[] }> }): Promise<Metadata> {
  try {
    ...
    return { title: `${displayTitle(title.title)} — FilmBase`, description: title.synopsis?.slice(0, 160) || `View download offers for ${displayTitle(title.title)} on FilmBase.`, openGraph: { images: image ? [{ url: image }] : [] } }
  } catch { return { title: "Title file — FilmBase" } }
}
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="site-shell pb-16 pt-24 sm:pt-28">
        ...
        {!hasAdFreeStream ? <AdSlot placement="detail-inline" className="my-10" /> : null}
        ...
        {!hasAdFreeStream ? <AdSlot placement="download-after" className="mt-12" /> : null}
        <aside className="mx-auto mt-12 flex max-w-5xl items-start gap-3 border-t border-border pt-5 text-xs leading-5 text-muted-foreground"><ExternalLink className="mt-0.5 h-4 w-4 shrink-0" /><p>External hosts control their own availability and file delivery. Check the filename and size before saving.</p></aside>
