...
type TmdbDetails = { id: number; runtime?: number | null; vote_average?: number; genres?: Array<{ name: string }>; overview?: string; poster_path?: string | null; backdrop_path?: string | null }
...
export async function enrichWithTmdb(items: CatalogTitle[], limit = 12): Promise<CatalogTitle[]> {
  ...
  await Promise.all(candidates.map(async (item) => {
    try {
      ...
      if (!match) return
      const detail = await fetchJson<TmdbDetails>(query(`/${kind}/${match.id}?language=en-US`), { headers: headers(), next: { revalidate: 86400 }, provider: "tmdb", timeoutMs: 3500 })
